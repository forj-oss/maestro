...
require 'puppet/provider/pinasdns/actions_record' if Puppet.features.pinas?

Puppet::Type.type(:pinasdns).provide(:record) do
  desc "Creates hp dns type for Gardner."
  has_feature :manage_record
  confine :feature => :fog
  confine :feature => :json
  confine :feature => :pinas
  Puppet.debug("loaded provider manage_recrod")
  include ::Pinas::DNS::ActionsRecord
