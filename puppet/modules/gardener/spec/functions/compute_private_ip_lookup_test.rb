...
describe "apply test compute_private_ip_lookup", :if => true, :apply => true do
  context 'with puppet apply' do
    it "should find a node called serverupnode1.42" do
      apply("notice(compute_private_ip_lookup('serverupnode1.42'))").should be(true)
    end
  end
end

describe "apply test compute_private_ip_lookup with fake node", :if => false, :apply => true do
  context 'with puppet apply' do
    it "should not find a node called foo" do
      apply("notice(compute_private_ip_lookup('foo'))").should be(false)
    end
  end
end