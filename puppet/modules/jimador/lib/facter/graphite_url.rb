...
Facter.add("graphite_url") do
 ...
 setcode do
   ...
   if File.exist? "/etc/statsd/config.js"
     ...
     Facter::Util::Resolution.exec("echo http://#{http_name}:$(egrep -lir \"graphite\" /etc/apache2/sites-enabled/*|xargs -i grep '<VirtualHost.*>' {}|awk -F'[<|>]' '{printf $2}'|awk -F':' '{printf $2}')")
   else
