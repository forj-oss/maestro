...
def check_FS_stats(path, yyaml):
    ...
    if 'status' in yyaml and 'filesystems' in yyaml['status']:
        if mount_point not in yyaml['status']['filesystems']:
            ...
            yyaml['status']['filesystems'][mount_point] = {'free-GB': free_avail,
    ...
    try:
        ...
        try:
            ...
            if type(back_status) is not dict:
                ...
                if type(back_status) is dict and 'errors' in back_status:
                    errors += back_status['errors']
                if type(back_status) is dict and 'warnings' in back_status:
...
def build_backup_status(bck_path, bck_file):
    ...
    ybck_status = {'status': {'week': '{0}-{1}'.format(run_year, run_week),
                              'date': run_date,
                              'filesystems': {},
                              'status': 0
                              },
    ...
    for root, dirnames, files in os.walk(bck_path):
        ...
        bIgnored = False
        if rel_path[0] != "" and reIgnored.search(rel_path[0]) is not None:
            ...
            for i in sorted(dirnames):
                dirnames.remove(i)
        elif rel_path[0] != "" and reSupportedName.search(rel_path[0]) is None:
        ...
        if not bIgnored and len(rel_path) == 3:  # server/service/week level
            ...
            if not bIgnored:
                ...
                if Server not in ybck_status['backup']:
                    ybck_status['backup'][Server] = {}
                if Service not in ybck_status['backup'][Server]:
                    ybck_status['backup'][Server] = {Service: {'status': 0,
                ...
                if Service not in ybck_status['services']:
            ...
            if 'last' not in yService or yService['last'] is None:
                yService['status'] = 2
                ...
                if iDaysOld == 1:
                    UpdateStatus(ybck_status['status'], 1, "Warning! {0}: Missing previous day backup. Please review service status.".format(Service))
                elif iDaysOld > 1:
                    UpdateStatus(ybck_status['status'], 2, "Error! {0}: Several backup days missing. Please review service status. If this service is obsolete, move '{0}' to '{1}'".format(Service, os.path.join(bck_path, 'ignored')))
    ...
    if 'message' not in ybck_status['status']:
