    ...
    var get_options = {};
      getInstanceId(function(inst_err){
        ...
        if(process.env.http_proxy === undefined){
          ...
          sails.log.debug('is_registered: url='+url);
        ...
        var get_req = http.request(get_options, function(res) {
            ...
            res.on('data', function (chunk) {
                sails.log.debug('is_registered: result: '+JSON.parse(chunk));
        ...
        get_req.on('error', function (e) {
            if (e.code === "ECONNRESET") {
                console.error('Kit Registration API Error: '+e.message);
                err("Our central registration service is currently offline.  Please try again later.  If the problem persist, please contact us <a href=\"http://ea.forj.io/community/\">here</a>");   
            }else{
                console.error('Kit Registration API Error: ' + e.message);
                err(e);
            }
  ...
  do_register: function(name, email, err, result){
    genJSON(name, email, function(errors){
      ...
      if(process.env.http_proxy === undefined){
        ...
        sails.log.debug('do_register: url='+url);
        var proxy_url = url.parse(process.env.http_proxy);
        post_options = {
            host: proxy_url.hostname,
            port: proxy_url.port,
      ...
      var post_req = http.request(post_options, function(res) {
          res.setEncoding('utf8');
          res.on('data', function (chunk) {
              sails.log.debug('do_register: result: '+JSON.parse(chunk));
              result(JSON.parse(chunk));
          });
      });
  ...
  async.series({
    InstanceId: function(callback){
      getInstanceId(function(err_instance){
        console.error('Failed to parse the instance id of the kit for the registration process.');
    ...
    Nodes: function(callback){
      genJSONNodes(function(err_node){
        console.error('Failed to parse the nodes of the kit for the registration process.');
        callback(err_node);
  ...
  blueprint_utils.get_current_raw_bp(function(err_raw){
    ...
    res_tools['node'].forEach(function(node){
      async.series({
        ...
        if(async_err){
          console.error('Error retrieving the nodes: '+async_err.message);
          err(async_err);
        }else{
