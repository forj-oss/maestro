    ...
    var get_options = {};
      getInstanceId(function(inst_err){
        ...
        if(process.env.http_proxy === undefined){
          get_options = {
              host: config_yaml.registration.ip,
              port: config_yaml.registration.port,
              path: config_yaml.registration.resource_uri + 'is_registered/'+instance_id,
              method: 'GET',
          ...
          var url_get = config_yaml.registration.protocol +'://'+config_yaml.registration.ip + ':'+ config_yaml.registration.port +''+ config_yaml.registration.resource_uri + 'is_registered/'+instance_id;
    ...
    genJSON(name, email, function(errors){
      ...
      if(process.env.http_proxy === undefined){
        post_options = {
            host: config_yaml.registration.ip,
            port: config_yaml.registration.port,
            path: config_yaml.registration.resource_uri + 'register',
            method: 'POST',
            rejectUnauthorized: false,
            requestCert: true,
            agent: false,
        ...
        var url_post = config_yaml.registration.protocol +'://'+config_yaml.registration.ip + ':'+ config_yaml.registration.port +''+ config_yaml.registration.resource_uri + 'register';
        var proxy_url = url.parse(process.env.http_proxy);
        post_options = {
            host: proxy_url.hostname,
            port: proxy_url.port,
