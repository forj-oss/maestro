...
/**
 * grab settings from config/{env}.js based on NODE_ENV var (see config/local.js)
 */
var da_host = sails.config.kit.registration.ip;
var da_port = sails.config.kit.registration.port;
var da_resource_uri = sails.config.kit.registration.resource_uri;
var da_protocol = sails.config.kit.registration.protocol;
      ...
      getInstanceId(function(inst_err){
        ...
        if(process.env.http_proxy === undefined){
          get_options = {
              host: da_host,
              port: da_port,
              path: da_resource_uri + 'is_registered/'+instance_id,
              method: 'GET',
          ...
          var url_get = da_protocol +'://'+ da_host + ':'+ da_port +''+ da_resource_uri + 'is_registered/'+instance_id;
  ...
  do_register: function(name, email, err, result){
    genJSON(name, email, function(errors){
      ...
      if(process.env.http_proxy === undefined){
        ...
        var url_post = da_protocol +'://'+ da_host + ':'+ da_port +''+ da_resource_uri + 'register';
        var proxy_url = url.parse(process.env.http_proxy);
        post_options = {
            host: proxy_url.hostname,
            port: proxy_url.port,
