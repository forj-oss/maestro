    ...
    var get_options = {};
      getInstanceId(function(inst_err){
        ...
        if(process.env.http_proxy === undefined){
          get_options = {
              host: sails.config.kit.registration.ip,
              port: sails.config.kit.registration.port,
              path: sails.config.kit.registration.resource_uri + 'is_registered/'+instance_id,
              method: 'GET',
          ...
          var url_get = sails.config.kit.registration.protocol +'://'+ sails.config.kit.registration.ip + ':'+ sails.config.kit.registration.port +''+ sails.config.kit.registration.resource_uri + 'is_registered/'+instance_id;
    ...
    genJSON(name, email, function(errors){
      ...
      if(process.env.http_proxy === undefined){
        post_options = {
            host: sails.config.kit.registration.ip,
            port: sails.config.kit.registration.port,
            path: sails.config.kit.registration.resource_uri + 'register',
            method: 'POST',
            rejectUnauthorized: false,
            requestCert: true,
            agent: false,
        ...
        var url_post = sails.config.kit.registration.protocol +'://'+ sails.config.kit.registration.ip + ':'+ sails.config.kit.registration.port +''+ sails.config.kit.registration.resource_uri + 'register';
        var proxy_url = url.parse(process.env.http_proxy);
        post_options = {
            host: proxy_url.hostname,
            port: proxy_url.port,
