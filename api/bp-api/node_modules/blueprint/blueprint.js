...
var blueprint_utils = {
  ...
  create_blueprint: function(shortname, instance_id, tools, defect_tracker, auth, users, projects, documentation, notification, maestro_url, blueprint_name, global_manage_projects, err, result){
    Bp.create({
      shortname: shortname,
      id: instance_id,
      tools: tools,
      ...
      global_manage_projects: global_manage_projects
    }).exec(function(err_, bp) {
      // Error handling
      if (err_) {
        err(err_);
  ...
  get_kit_blueprint: function(err, blueprint){
    ...
    if(config  instanceof Error){
      err('get_kit_blueprint: Error reading the configuration file.');
    }else{
      blueprint_utils.get_blueprint(JSON.parse(config).site.id, function(errbp){
        ...
        if(bp === undefined){
          ...
          blueprint_utils.create_blueprint(config.site.shortname, config.site.id, tools, config.site.defect_tracker, config.site.auth, config.site.users, config.site.projects, config.site.documentation, config.site.notification, config.site.maestro_url, config.site.blueprint, config.site.global_manage_projects, function(errc){
            err(errc);
          }, function(bpc){
            blueprint(bpc);
          });
  ...
  get_kit_blueprint_sync: function(err, blueprint){
    var config =  require('config/config').get_config_data();
    if(config  instanceof Error){
      err('get_kit_blueprint_sync: Error reading the configuration file.');
    }else{
      blueprint_utils.get_blueprint(JSON.parse(config).site.id, function(errbp){
        if(errbp){
          err(errbp);
