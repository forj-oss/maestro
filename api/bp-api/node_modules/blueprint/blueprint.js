var blueprint_utils = {
  ...
  create_blueprint: function(instance_id, tools, defect_tracker, auth, users, projects, documentation, maestro_url, err, result){
    Bp.create({
      id: instance_id,
      tools: tools,
    ...
    if(config  instanceof Error){
      ...
      blueprint_utils.get_blueprint(JSON.parse(config).site.id, function(errbp){
        ...
        if(bp === undefined){
          ...
          blueprint_utils.create_blueprint(config.site.id, tools, config.site.defect_tracker, config.site.auth, config.site.users, config.site.projects, config.site.documentation, config.site.maestro_url, function(errc){
            err(errc);
          }, function(bpc){
            blueprint(bpc);
          });
